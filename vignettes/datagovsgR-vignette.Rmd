...
This functions calls upon the UVI API from data.gov.sg and returns the UVI value for the given data-time. This data provided by the API is updated hourly. There is only information on the UVI between 0700 and 1900. Entering a date-time outside of the range would return the UVI of the closest date time instead.  There are 2 functions, the `uvi` which returns the data for a given date-time, and `uvi_summary` which returns the data summary for a given date.
...
```{r}
uvi_summary(date = "2019-04-01") %>% head()
```
